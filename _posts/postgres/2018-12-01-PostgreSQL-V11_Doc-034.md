...
811libpq — библиотека для языка C
...
тём `pg_config --sysconfdir`/pg_service.conf или в каталоге, задаваемом переменной окруже-
...
сания службы в pg_service.conf может быть таким:
...
ни сертификата начинается со звёздочки (*), звёздочка воспринимается как подстановочный знак
...
826
We use select(2)libpq — библиотека для языка C
...
827libpq — библиотека для языка C
